...
class getPages:
    ...
    def getPage(self, pageNumber):
            try:
                opener = urllib2.build_opener()
                opener.addheaders = [('User-agent', 'Mozilla/5.0')]
                url = "http://www.bestbuy.ca/en-ca/category/printers-scanners-fax/20330.aspx?type=product&NVID=shop%3BComputers%2C+Tablets+%26+Accessories%3BPrinters%2C+Scanners+%26+Fax%3Blnk%3Bc2%3Br10%3Ben&page=" + str(pageNumber) + "&pageSize=64"
                print "entered"
                print url
...
df["Images"] = image
print modelNumbers
df.to_csv("BestBuyPrinters.csv",index=False, encoding='utf-8')
