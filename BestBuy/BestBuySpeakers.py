...
class getPages:
    ...
    def getPage(self, pageNumber):
            try:
                opener = urllib2.build_opener()
                opener.addheaders = [('User-agent', 'Mozilla/5.0')]
                url = "http://www.bestbuy.ca/en-ca/category/home-audio-speakers/20303.aspx?type=product&page=" + str(pageNumber) + "&pageSize=64"
                print "entered"
                print url
            ...
            for ul in all_ul:
                ...
                for div in ul.findAll("div", {"class":"prod-info"}):
                    ...
                    with scrapingThread.add_lock:
                        modelNumberP, images = findModel(urlLink)
                        name = h4.text
                        ...
                        cursor.execute(
                            ...
                            (name, prices, urlLink, modelNumberP, images, vendor))
                        image.append(images)
                        title.append(h4.text)
                        modelNumber.append(model)
                        link.append(urlLink)
...
df["Images"] = image
print modelNumber
df.to_csv("BestBuySpeakers.csv",index=False, encoding='utf-8')
