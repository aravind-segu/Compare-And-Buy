...
class getPages:
    ...
    def getPage(self, pageNumber):
            try:
                opener = urllib2.build_opener()
                opener.addheaders = [('User-agent', 'Mozilla/5.0')]
                url = "http://www.bestbuy.ca/en-ca/category/cameras/cameras.aspx?type=product&NVID=shop%3BCameras%2C+Camcorders+%26+Drones%3BCameras%3Blnk%3Bc1%3Br1%3Ben&page=" + str(
                   pageNumber) + "&pageSize=64"
                print "entered"
                print url
...
df["Images"] = image
print modelNumbers
df.to_csv("BestBuyCameras.csv",index=False, encoding='utf-8')
