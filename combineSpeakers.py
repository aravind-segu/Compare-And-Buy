...
combinedData = pd.read_sql("SELECT * FROM speakers", con= conn)

bestBuy = pd.read_sql("SELECT * FROM speakers WHERE VENDOR='bestbuy'", con= conn)
print bestBuy.shape

amazon = pd.read_sql("SELECT * FROM speakers WHERE VENDOR='amazon'", con= conn)
print amazon.shape

source = pd.read_sql("SELECT * FROM speakers WHERE VENDOR='source'", con= conn)
print source.shape

newEgg = pd.read_sql("SELECT * FROM speakers WHERE VENDOR='newegg'", con= conn)
print newEgg.shape

snapDeal = pd.read_sql("SELECT * FROM speakers WHERE VENDOR='snapdeal'", con= conn)
print snapDeal.shape

staples = pd.read_sql("SELECT * FROM speakers WHERE VENDOR='staples'", con= conn)
print staples.shape

walmart = pd.read_sql("SELECT * FROM speakers WHERE VENDOR='walmart'", con= conn)
print walmart.shape
...
combinedBrandDataset = [dSamsung, dSony, dBose, dFocal, dSonus, dBoss, dPolk, dOther]
companyNames = []
...
groupByBrand(combinedDataset, dBose, companyNames, "bose")
groupByBrand(combinedDataset, dFocal, companyNames, "focal")
groupByBrand(combinedDataset, dSonus, companyNames, "sonus")
groupByBrand(combinedDataset, dBoss, companyNames, "boss")
groupByBrand(combinedDataset, dPolk, companyNames, "polk")
...
for masterDict in combinedMaster:
    ...
    for key in masterDict:
        ...
        try:
            cursor.execute(
                "INSERT into speakercombination(Name,Rating) VALUES ('%s', '%s')" % \
                (key, rating))
        except:
            print "error while inserting into speakrcombo"
            continue
    ...
    for key in masterDict:
        try:
            ...
            for currid in productIDs:
                try:
                    cursor.execute("UPDATE speakers SET speakerMapping=%d WHERE ID=%d" % (insertedId, currid))
                except:
                    print "error while inserting into speakers"
                    continue
        except:
            print "wrong key"
            continue
...
with open('SpekersMaster.csv', 'wb') as csv_file:
