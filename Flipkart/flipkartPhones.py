...
class getPages:
    ...
    def getPage(self, pageNumber):
            try:
                url = "https://www.flipkart.com/mobiles/pr?otracker=categorytree&page="+str(pageNumber)+"&sid=tyy%2C4io&viewType=list"
                print url
                r = requests.get(url)
...
class scrapingThread (threading.Thread):
    ...
    def run(self):
        while(True):
            ...
            if scrapingThread.pageNumber > 50:
                break
            else:
...
df.to_csv("FlipkartPhones.csv",index=False, encoding='utf-8')