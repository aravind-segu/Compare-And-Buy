...
class getPages:
    ...
    def getPage(self, pageNumber):
            try:
                url = "https://www.flipkart.com/search?page=" + str(pageNumber) + "&sid=tyy%2Fhry&viewType=list"
                print url
                r = requests.get(url)
...
class scrapingThread (threading.Thread):
    ...
    def run(self):
        while(True):
            ...
            allProducts = soup.find("div", {"data-reactid":"339"})
            if not allProducts:
                break
            else:
    ...
    def getInformation(self,soup):
        ...
        try:
            allProducts = soup.find("div", {"data-reactid": "339"})
            divs = allProducts.findAll("div", {"class": "_2-gKeQ"})
            for div in divs:
                link = div.find("a")
...
df.to_csv("FlipkartTablets.csv",index=False, encoding='utf-8')