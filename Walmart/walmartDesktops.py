...
class scrape():
    ...
    def getInformation(self, pageNumber, threadNum):
        ...
        if threadNum is 5:
            browser = browser5
        browser.get("https://www.walmart.ca/en/electronics/laptops-computers/desktop-computers/N-1850/page-" + str(pageNumber))
        time.sleep(1)
        html = browser.page_source
        soup = BeautifulSoup(html)
...
browserModel5 = webdriver.Chrome("../chromedriver.exe")
sc = scrape()
...
t5 = threading.Thread(target=scrape.getInformation, args=(sc, 5, 5,))
t1.start()
t2.start()
t3.start()
t4.start()
...
df["Images"] = images
df.to_csv("walmartDesktops.csv", index=False, encoding='utf-8')
