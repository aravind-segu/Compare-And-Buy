...
combinedData = pd.read_sql("SELECT * FROM phones", con= conn)

bestBuy = pd.read_sql("SELECT * FROM phones WHERE VENDOR='bestbuy'", con= conn)
print bestBuy.shape

amazon = pd.read_sql("SELECT * FROM phones WHERE VENDOR='amazon'", con= conn)
print amazon.shape

newEgg = pd.read_sql("SELECT * FROM phones WHERE VENDOR='newegg'", con= conn)
print newEgg.shape

flipkart = pd.read_sql("SELECT * FROM phones WHERE VENDOR='flipkart'", con= conn)
print flipkart.shape

snapDeal = pd.read_sql("SELECT * FROM phones WHERE VENDOR='snapdeal'", con= conn)
print snapDeal.shape

staples = pd.read_sql("SELECT * FROM phones WHERE VENDOR='staples'", con= conn)
print staples.shape

walmart = pd.read_sql("SELECT * FROM phones WHERE VENDOR='walmart'", con= conn)
print walmart.shape
...
brands.append("motorola")
brands.append("huawei")
brands.append("htc")
brands.append("zte")
brands.append("other")
...
dZte = {}
dOther = {}
combinedBrandDataset = [dSamsung, dAsus, dLg, dGoogle, dMotorola, dHuawei, dHtc, dZte, dOther]
companyNames = []
...
groupByBrand(combinedDataset, dGoogle, companyNames, "google")
groupByBrand(combinedDataset, dMotorola, companyNames, "motorola")
groupByBrand(combinedDataset, dHuawei, companyNames, "huawei")
groupByBrand(combinedDataset, dHtc, companyNames, "htc")
groupByBrand(combinedDataset, dZte, companyNames, "zte")
...
for masterDict in combinedMaster:
    ...
    for key in masterDict:
        ...
        try:
            cursor.execute(
                "INSERT into phonescombination(Name,Rating) VALUES ('%s', '%s')" % \
                (key, rating))
        except:
            print "error while inserting into phonesCombo"
            continue
    ...
    for key in masterDict:
        try:
            ...
            for currid in productIDs:
                try:
                    cursor.execute("UPDATE phones SET phoneMapping=%d WHERE ID=%d" % (insertedId, currid))
                except:
                    print "error while inserting into phones"
                    continue
        except:
            print "wrong key"
            continue
...
with open('PhonesMaster.csv', 'wb') as csv_file:
