...
class scrapingThread (threading.Thread):
    ...
    def getInformation(self, soup):
        ...
        try:
            ...
            for div in all_divs:
                image = div.find("div", class_="product-image")
                ...
                with scrapingThread.addLock:
                    vendor = "staples"
                    cursor.execute("INSERT into tablets (Name, Price, Link, ModelNumber, Images, Vendor) VALUES('%s', '%s', '%s', '%s', '%s', '%s')" % (productName,productPrice,productLink,modelNumberPro,imageLink,vendor))
                    print imageLink
...
df["Image"] = images
df.to_csv("StaplesTablets.csv", index=False, encoding='utf-8')
