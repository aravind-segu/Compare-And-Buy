...
class getPages:
    def getPage(self, pageNumber):
        try:
            opener = urllib2.build_opener()
            opener.add_handlers  = [('User-agent', 'Mozilla/5.0')]
            url = "https://www.thesource.ca/en-ca/computers-and-tablets/printers-and-scanners/inkjet-printers/c/scc-1-7-3?q=%3Arelevance&page=" + str(pageNumber) + "&view=grid"
            print url
...
df["Images"] = images
oldSpeakers = pd.read_csv("SourcePrinters.csv")
...
master.to_csv("SourcePrinters.csv",index=False, encoding='utf-8')
