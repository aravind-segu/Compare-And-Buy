...
class getPages:
    def getPage(self, pageNumber):
        try:
            opener = urllib2.build_opener()
            opener.add_handlers  = [('User-agent', 'Mozilla/5.0')]
            url = "https://www.thesource.ca/en-ca/computers-and-tablets/printers-and-scanners/all-in-one-printers/c/scc-1-7-1?q=%3Arelevance&page="+ str(pageNumber)+"&view=grid"
            print url
        ...
        try:
            ...
            for div in all_div:
                ...
                with scrapingThread.add_lock:
                    ...
                    mapping = 1
                    cursor.execute(
                        "INSERT into printers(Name, Price, Link, ModelNumber, Images, Vendor, tvMapping) VALUES ('%s', '%s', '%s', '%s','%s', '%s', %d)" % \
...
df["Images"] = images
df.to_csv("SourcePrinters.csv",index=False, encoding='utf-8')
