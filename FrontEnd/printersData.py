...
tabletsRelease = pd.read_sql("SELECT * FROM printercombination ORDER BY Rating ASC", con=conn)
...
def getPrinters():
    ...
    for j, row in tabletsRelease.iterrows():
        ...
        indTablets = []
        duplicateDict = {}
        products = pd.read_sql("SELECT * FROM printers WHERE printerMapping = %(idValue)s", con = conn, params = {"idValue":idNumber})
    ...
    return tabletsList
def getPrintersPage(tabletsList, pageNumber):
    if pageNumber == 1:
...
def getMaxPagePrinter(tabletsList):
