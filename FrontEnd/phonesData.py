...
tabletsRelease = pd.read_sql("SELECT * FROM phonescombination ORDER BY Rating ASC", con=conn)
...
def getPhones():
    ...
    for j, row in tabletsRelease.iterrows():
        ...
        indTablets = []
        duplicateDict = {}
        products = pd.read_sql("SELECT * FROM phones WHERE phoneMapping = %(idValue)s", con = conn, params = {"idValue":idNumber})
    ...
    return tabletsList
def getPhonesPage(tabletsList, pageNumber):
    if pageNumber == 1:
...
def getMaxPagePhone(tabletsList):
