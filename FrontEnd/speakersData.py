...
tabletsRelease = pd.read_sql("SELECT * FROM speakercombination ORDER BY Rating ASC", con=conn)
...
def getSpeakers():
    ...
    for j, row in tabletsRelease.iterrows():
        ...
        indTablets = []
        duplicateDict = {}
        products = pd.read_sql("SELECT * FROM speakers WHERE speakerMapping = %(idValue)s", con = conn, params = {"idValue":idNumber})
    ...
    return tabletsList
def getSpeakersPage(tabletsList, pageNumber):
    if pageNumber == 1:
...
def getMaxPageSpeaker(tabletsList):
