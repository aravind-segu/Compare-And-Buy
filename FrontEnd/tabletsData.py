...
def getTablets():
    ...
    tabletsRelease = pd.read_sql("SELECT * FROM tabletscombination ORDER BY Rating ASC", con=conn)
    tabletsList = []
    count = 0
    for j, row in tabletsRelease.iterrows():
        ...
        count += 1
        indTablets = []
        products = pd.read_sql("SELECT * FROM tablets WHERE tabletMapping = %(idValue)s", con = conn, params = {"idValue":idNumber})
        duplicateDict = {}
    ...
    return tabletsList
def getTabletsPage(tabletsList, pageNumber):
    if pageNumber == 1:
