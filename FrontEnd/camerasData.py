...
tabletsRelease = pd.read_sql("SELECT * FROM camerascombination ORDER BY Rating ASC", con=conn)
...
def getCameras():
    ...
    for j, row in tabletsRelease.iterrows():
        ...
        indTablets = []
        duplicateDict = {}
        products = pd.read_sql("SELECT * FROM cameras WHERE cameraMapping = %(idValue)s", con = conn, params = {"idValue":idNumber})
    ...
    return tabletsList
def getCamerasPage(tabletsList, pageNumber):
    if pageNumber == 1:
...
def getMaxPageCamera(tabletsList):
