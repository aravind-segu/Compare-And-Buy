...
tabletsRelease = pd.read_sql("SELECT * FROM desktopscombination ORDER BY Rating ASC", con=conn)
...
def getDesktops():
    ...
    for j, row in tabletsRelease.iterrows():
        ...
        indTablets = []
        duplicateDict = {}
        products = pd.read_sql("SELECT * FROM desktops WHERE desktopMapping = %(idValue)s", con = conn, params = {"idValue":idNumber})
    ...
    return tabletsList
def getDesktopsPage(tabletsList, pageNumber):
    if pageNumber == 1:
...
def getMaxPageDesktop(tabletsList):
